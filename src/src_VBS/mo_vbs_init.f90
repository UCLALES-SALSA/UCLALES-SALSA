...
SUBROUTINE init_vbs(nvbs_setup, laqsoa)
    USE mo_vbs, ONLY : vbs_species, voc_id_isop, voc_id_mtp
    ...
    USE mo_submctl, ONLY : dens, mws, diss, nspec, zspec, moc, dissoc, rhooc, &
            nvocs, nvbs, naqsoa, mws_gas, ngases, zgas, conc_voc, id_isop, id_mtp, &
    ...
    CALL vbs_species(nvbs_setup, laqsoa, rhooc, moc, kappaoc, conc_voc)
    ...
    ! a) Oxidants (OH, O3 and NO3) - either prognostic or diagnostic (constant concentration)
    IF (conc_oh>=0.) THEN
    ...
    DO i=1, vbs_nvocs
        ngases = ngases + 1
        mws_gas(ngases)=vbs_voc_set(i)%mw*1e-3
        WRITE(tmp,"('VO',I1)") i
        zgas(ngases)=tmp
        ! Isoprene and monoterpene indexes for surface emissions
        IF (voc_id_isop==i) id_isop=ngases
        IF (voc_id_mtp==i) id_mtp=ngases
    ENDDO
    ! c) VBS species
    DO i=1, vbs_ngroup
        ngases = ngases + 1
        mws_gas(ngases)=vbs_set(i)%mw*1e-3
        WRITE(tmp,"('VB',I1)") i
        zgas(ngases)=tmp
    ENDDO
    ! d) aqSOA species
    DO i=1, aqsoa_ngroup
        ngases = ngases + 1
        mws_gas(ngases)=aqsoa_set(i)%mw*1e-3
        WRITE(tmp,"('AQ',I1)") i
        zgas(ngases)=tmp
    ENDDO
    ...
    DO i=1,vbs_ngroup
        ...
        dens(j)=vbs_set(i)%dens
        mws(j)=vbs_set(i)%mw*1e-3
        ! kappa-Kohler: k=diss_i*v_w/v_i => diss_i=k*v_i/v_w, where v=M/rho
        diss(j)=vbs_set(i)%kappa*( mws(j)/dens(j) )/( mws(1)/dens(1) )
        ! SALSA/LES name
        WRITE(tmp,"('VB',I1)") i
        zspec(j)=tmp
    ENDDO
    ...
    DO i=1,aqsoa_ngroup
        nspec=nspec + 1
        j=nspec+1
        dens(j)=aqsoa_set(i)%dens
        mws(j)=aqsoa_set(i)%mw*1e-3
        diss(j)=aqsoa_set(i)%kappa*( mws(j)/dens(j) )/( mws(1)/dens(1) )
        WRITE(tmp,"('AQ',I1)") i
        zspec(j)=tmp
    ENDDO
