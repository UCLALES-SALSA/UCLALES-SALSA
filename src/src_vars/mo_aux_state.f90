MODULE mo_aux_state
  ...
  CONTAINS
    ...
    SUBROUTINE setInitialProfiles(BasicState,nzp)
      ...
      n1 = 1; n2 = nzp
      pipeline => NULL()
      u0 = FloatArray1d("u0",trgt=a_basicstate(n1:n2))
      pipeline => u0
      CALL BasicState%newField(u0%shortName,'basic state u wind','m/s','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      v0 = FloatArray1d("v0",trgt=a_basicstate(n1:n2))
      pipeline => v0
      CALL BasicState%newField(v0%shortName,'basic state v wind','m/s','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      pi0 = FloatArray1d("pi0",trgt=a_basicstate(n1:n2))
      pipeline => pi0
      CALL BasicState%newField(pi0%shortName,'pi0','Pa','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      pi1 = FloatArray1d("pi1",trgt=a_basicstate(n1:n2))
      pipeline => pi1
      CALL BasicState%newField(pi1%shortName,'pi1','Pa','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      th0 = FloatArray1d("th0",trgt=a_basicstate(n1:n2))
      pipeline => th0
      CALL BasicState%newField(th0%shortName,'basic state potential temperature','K','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      dn0 = FloatArray1d("dn0",trgt=a_basicstate(n1:n2))
      pipeline => dn0
      CALL BasicState%newField(dn0%shortName,'basic state air density','kg/m3','zt',.FALSE.,pipeline)

      n1 = n1+nzp; n2 = n2+nzp
      pipeline => NULL()
      rt0 = FloatArray1d("rt0",trgt=a_basicstate(n1:n2))
      pipeline => rt0
      CALL BasicState%newField(rt0%shortName,'basic state specific humidity','kg/kg','zt',.FALSE.,pipeline)
    ...
    SUBROUTINE setGridSpacings(Axes,level,nzp,nxp,nyp)
      ...
      IF (level >= 4) THEN
         ...
         n1aux = n1
         pipeline => NULL()
         aea = FloatArray1d("aea",trgt=a_grid(n1:n2))
         pipeline => aea
         ...
         n2aux = n2
         pipeline => NULL()
         aeb = FloatArray1d("aeb",trgt=a_grid(n1:n2))
         pipeline => aeb
         CALL Axes%newField('aeb','Aerosol B lower limits','m','aeb',.TRUE.,pipeline,in_group=["ps","ts"])

         pipeline => NULL()
         aetot = FloatArray1d("aetot",trgt=a_grid(n1aux:n2aux))
         pipeline => aetot
         ...
         n1aux = n1
         pipeline => NULL()
         cla = FloatArray1d("cla",trgt=a_grid(n1:n2))
         pipeline => cla
         ...
         IF (level == 5) THEN
            n1 = n2 + 1; n2 = n2 + nice
            pipeline => NULL()
            ice = FloatArray1d("ice",trgt=a_grid(n1:n2))     
            pipeline => ice
