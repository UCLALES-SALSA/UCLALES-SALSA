...
module fuliou
  ...
  use cldwtr, only : init_cldwtr, cloud_water, init_cldice, cloud_ice, init_cldgrp, cloud_grp, init_aerorad, aero_rad
  use solver, only : qft
  use ckd
...
contains
  ...
  subroutine rad (as, u0, ss, pts, ee, pp, pt, ph, po, fds, fus, fdir, fuir, &
       McICA, laerorad, plwc, pre, piwc, pde, prwc, pgwc, maerobin, naerobin, prtcl)
    ...
    REAL, OPTIONAL, INTENT(in) :: maerobin(nv,(nspec+1)*nbins),  & ! 
                                  naerobin(nv,nbins)     !
    ...
    call rad_ir(pts, ee, pp, pt, ph, po, fdir, fuir, McICA, laerorad, &
                 plwc, pre, piwc, pde, prwc, pgwc, maerobin, naerobin, prtcl)

    call rad_vis(as, u0, ss, pp, pt, ph, po, fds, fus, McICA, laerorad, &
                 plwc, pre, piwc, pde, prwc, pgwc, maerobin, naerobin, prtcl )
  ...
  ! defined by input ckd file
  !
  subroutine rad_ir (pts, ee, pp, pt, ph, po, fdir, fuir, McICA, laerorad, &
       plwc, pre, piwc, pde, prwc, pgwc, maerobin, naerobin, prtcl )
    ...
    REAL, OPTIONAL, INTENT(in) :: maerobin(nv,(nspec+1)*nbins), naerobin(nv,nbins)
    TYPE(ComponentIndex), OPTIONAL, INTENT(in) :: prtcl 
    ...
    bandLoop: do ibandloop = 1, iblimit
      ...
      IF ( laerorad ) THEN
         CALL aero_rad(ib + size(solar_bands), nbins, nchem, maerobin, naerobin, &
                       prtcl, dz, taer, waer, wwaer                              )
  ...
  subroutine rad_vis (as, u0, ss, pp, pt, ph, po, fds, fus, McICA, laerorad, &
       plwc, pre, piwc, pde, prwc, pgwc, maerobin, naerobin, prtcl )
    ...
    REAL, OPTIONAL, INTENT(in) :: maerobin(:,:), naerobin(:,:)
    TYPE(ComponentIndex), OPTIONAL, INTENT(in) :: prtcl
    ...
    if(u0 > minSolarZenithCosForVis) then
      ...
      bandLoop: do ibandloop =  1, iblimit
         ...
         IF (laerorad) THEN
            CALL aero_rad(ib, nbins, nchem, maerobin, naerobin, prtcl, dz, taer, waer, wwaer)
            CALL combineOpticalProperties(TauNoGas, wNoGas, pfNoGas, taer, waer, wwaer)
         END IF
