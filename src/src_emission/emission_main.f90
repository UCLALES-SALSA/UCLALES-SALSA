MODULE emission_main
  ...
  SUBROUTINE init_emission
    ...
    IF ( eseed%lemit ) THEN
       ...
       IF ( .NOT. spec%isUsed(eseed%species) ) THEN
          CALL errorMessage(global_name, name, &
                            'Attempt to emit <'//TRIM(eseed%species)//'> but the compound '// &
  ...
  SUBROUTINE cloud_seeding()
    ...
    nc = spec%getIndex(emit_spec) ! Index for emitted species, needed in several lines
    ...
    ! The line below (non-zero water content in emitted aerosol) is needed because of the current restrictions in SALSA's condensation routine.
