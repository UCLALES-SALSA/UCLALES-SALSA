...
module radiation
  ...
  contains
    ...
    subroutine d4stream(n1, n2, n3, alat, time, sknt, sfc_albedo, dn0, pi0, pi1, dzm, &
           pip, tk, rv, rc, cdnc, tt, rflx, sflx, albedo, ice, nice, rr, grp, radsounding, useMcICA)

      integer, intent (in) :: n1, n2, n3
      real, intent (in)    :: alat, time, sknt, sfc_albedo
      real, dimension (n1), intent (in)                 :: dn0, pi0, pi1, dzm
      real, dimension (n1,n2,n3), intent (in)           :: pip, tk, rv, rc, cdnc
      real, optional, dimension (n1,n2,n3), intent (in) :: ice,nice,rr,grp
      ...
      do j=3,n3-2
         do i=3,n2-2
            ...
            do k=2,n1
               ...
               pt(kk) = tk(k,i,j) ! Temperature
               ph(kk) = rv(k,i,j) ! Water vapor

               ! Liquid droplets
               plwc(kk) = 1000.*dn0(k)*max(0.,rc(k,i,j))
               if (plwc(kk).gt.0.) THEN
                  pre(kk)  = 1.e6*(plwc(kk)/(1000.*prw*cdnc(k,i,j)*dn0(k)))**(1./3.)
