MODULE constrain_SALSA
  ...
  CONTAINS
            ...
            DO kk = 1, nzp
               ...
               DO cc = 1,nice
                  ...
                  IF ( a_nicep(kk,ii,jj,cc)+a_nicet(kk,ii,jj,cc)*dtlt < 0. ) THEN
                     ...
                     END DO
                     ! Rime; the tendency cannot be larger than that for total ice in absolute values
   ...
   SUBROUTINE SALSA_diagnostics(callno)
    ...
    ! Check that rimed ice is smaller than the total ice and truncate rimed if necessary. Print a warning if rimed ice is significantly larger than total ice
    DO bc = iia%cur,fcb%cur
       mi = getMassIndex(nice,bc,nspec)  ! Total ice
       mi2 = getMassIndex(nice,bc,nspec+1) ! Rimed ice

       IF ( ANY( a_micep(:,:,:,mi) - a_micep(:,:,:,mi2) < -1.e-8 ) ) &
            WRITE(*,*) "SALSA_diagnostics warning: Rimed ice much larger than total ice", callno
       
       a_micep(:,:,:,mi2) = MIN(a_micep(:,:,:,mi2), a_micep(:,:,:,mi))
       ...
       DO i = 3,nxp-2
          DO k = 1,nzp
             ...
             DO bc = 1,nice
                ...
                mi2 = getMassIndex(nice,bc,nspec+1)  ! For rime
                IF (a_nicep(k,i,j,bc) > 0. .AND. SUM(a_micep(k,i,j,bc:mi:nice)) <= 0.) THEN
                   a_nicep(k,i,j,bc) = 0.
             ...
             ! Snow
             DO bc = 1,nsnw
                mi = getMassIndex(nsnw,bc,nspec)
                IF (a_nsnowp(k,i,j,bc) > 0. .AND. a_msnowp(k,i,j,mi) <= 0.) THEN
                   a_nsnowp(k,i,j,bc) = 0.
                   a_msnowp(k,i,j,bc:mi:nsnw) = 0.
                END IF
             END DO ! nsnw
       ...
       DO j = 3,nyp-2
          DO i = 3,nxp-2
             DO k = 1,nzp
                ...
                mi = getMassIndex(ncld,bc,nspec)    ! all
                ...
                IF ( a_ncloudp(k,i,j,bc) > nlim .AND. a_rh(k,i,j)<0.999 .AND.   &
                   ...
                   IF ( zdh2o < MAX(0.2*cdcld,2.e-6) .OR.     &
                        a_mcloudp(k,i,j,mi) < 1.e-25*a_ncloudp(k,i,j,bc) )  THEN
          ...
          DO i = 3,nxp-2
             DO k = 1,nzp
                ...
                IF ( a_nprecpp(k,i,j,bc) > prlim .AND. a_rh(k,i,j)<0.999 .AND.  &
                   ...
                   ! Lose the droplets if smaller than 0.02*critical radius or 2 um // try 10um instead?
                   IF ( zdh2o < MAX(0.02*cdprc,10.e-6) .OR.   &
                        a_mprecpp(k,i,j,mi)<1e-25*a_nprecpp(k,i,j,bc) ) THEN
                      ...
                      zvol = SUM( a_mprecpp(k,i,j,bc:mi2:nprc) )/a_nprecpp(k,i,j,bc) ! Dry mass

                      ! 1) Find the closest matching bin based on dry particle diameter (a and b bins)
                      cd = SUM( a_mprecpp(k,i,j,bc:mi2:nprc)/spec%rholiq(1:nspec-1) ) / &
                           a_nprecpp(k,i,j,bc) ! Dry diameter
                      cd = (cd/pi6)**(1./3.)
                      ...
                      ELSE IF (a_naerop(k,i,j,ba) <= nlim) THEN
                         ...
                         ba = bb
                      ELSE
                         mi = getMassIndex(nbins,ba,nspec-1)
                         mi2 = getMassIndex(nprc,bc,nspec-1)
                         ! Both are present - find bin based on compositional similarity
                         ra = calc_correlation(a_maerop(k,i,j,ba:mi:nbins),  &
                                               a_mprecpp(k,i,j,bc:mi2:nprc),   &
                                               nspec-1)

                         mi = getMassIndex(nbins,bb,nspec-1)
                         rb = calc_correlation(a_maerop(k,i,j,bb:mi:nbins),  &
                                               a_mprecpp(k,i,j,bc:mi2:nprc),   &
                                               nspec-1)
                         IF (ra<rb) ba = bb
                      END IF                     
       ...
       DO j = 3,nyp-2
          DO i = 3,nxp-2
             DO k = 1,nzp
                ...
                mi = getMassIndex(nsnw,bc,nspec)
                mi2 = getMassIndex(nsnw,bc,nspec-1)
                IF ( a_nsnowp(k,i,j,bc) > prlim .AND. a_rhi(k,i,j)<0.999 .AND.  &
                     a_msnowp(k,i,j,mi) < 1.e-20 ) THEN

                   cd = SUM( a_msnowp(k,i,j,bc:mi:nsnw)/spec%rhosnow(1:nspec) ) / &
                        a_nsnowp(k,i,j,bc)
                   cd = (cd/pi6)**(1./3.)
                   ...
                   IF ( zvol>0.5 .OR. cd < 2.e-6) THEN
                      ...
                      ! 1) Find the closest matching bin based on dry particle diameter (a and b bins)
                      cd = SUM( a_msnowp(k,i,j,bc:mi2:nsnw)/spec%rhosnow(1:nspec-1) ) / &
                           a_nsnowp(k,i,j,bc)
                      cd = (cd/pi6)**(1./3.)
    ...
    ! Rimed ice
    str = getMassIndex(nice,iia%cur,nc+1)
    end = getMassIndex(nice,fib%cur,nc+1)
